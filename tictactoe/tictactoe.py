...
class DataWrite:
    ...
    def value_in_arr(self, cvs: list, xy: str) -> None:
        """input x and y"""
        try:
            self.xy = xy.split(" ")
    ...
    @staticmethod
    def choice_player(cvs: list, x: int, y: int) -> None:
        """check if player"""
        if x > 3 or y > 3:
...
class Canvas:
    ...
    def table(self) -> None:
        """table function"""
        for line_cvs in self.cvs:
            print(f' |{"".join([f" {value_cell} " for value_cell in line_cvs])}|')
